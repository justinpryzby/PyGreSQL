...
/*
   A buffer with routines to handle resizing.
   This is inspired by libpq's PQExpBufferData.
   But doesn't handle binary data.
*/
struct ExpBuffer {
    char *data;
    ...
    /* if there was an allocation error, the buffer contents are undefined */
    int error;
};
...
pg_connect(PyObject *self, PyObject *args, PyObject *dict)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, dict, "|zzizzzi", (char **)kwlist,
                                     &pgdbname, &pghost, &pgport, &pgopt,
                                     &pguser, &pgpasswd, &nowait)) {
        return NULL;
    }
...
pg_escape_bytea(PyObject *self, PyObject *data)
{
    ...
    to = (char *)PQescapeBytea((unsigned char *)from, (size_t)from_length,
                               &to_length);
...
{
    /* gets arguments */
    if (!PyArg_ParseTuple(args, "O!O!O!O!", &PyFunction_Type, &dictiter,
                          &PyFunction_Type, &namediter, &PyFunction_Type,
                          &namednext, &PyFunction_Type, &scalariter)) {
        return NULL;
    }
...
static char pg_set_jsondecode__doc__[] =
    "set_jsondecode(func) -- set a function to be used for decoding json "
    "results";
...
static char pg_set_defhost__doc__[] =
    "set_defhost(string) -- set default database host and return previous "
    "value";
...
static char pg_set_defbase__doc__[] =
    "set_defbase(string) -- set default database name and return previous "
    "value";
...
pg_cast_array(PyObject *self, PyObject *args, PyObject *dict)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, dict, "O|Oc", (char **)kwlist,
                                     &string_obj, &cast_obj, &delim)) {
        return NULL;
    }
...
static struct PyMethodDef pg_methods[] = {
    {"connect", (PyCFunction)pg_connect, METH_VARARGS | METH_KEYWORDS,
     pg_connect__doc__},
    {"escape_string", (PyCFunction)pg_escape_string, METH_O,
     pg_escape_string__doc__},
    {"escape_bytea", (PyCFunction)pg_escape_bytea, METH_O,
     pg_escape_bytea__doc__},
    {"unescape_bytea", (PyCFunction)pg_unescape_bytea, METH_O,
     pg_unescape_bytea__doc__},
    {"get_datestyle", (PyCFunction)pg_get_datestyle, METH_NOARGS,
     pg_get_datestyle__doc__},
    {"set_datestyle", (PyCFunction)pg_set_datestyle, METH_VARARGS,
     pg_set_datestyle__doc__},
    {"get_decimal_point", (PyCFunction)pg_get_decimal_point, METH_NOARGS,
     pg_get_decimal_point__doc__},
    {"set_decimal_point", (PyCFunction)pg_set_decimal_point, METH_VARARGS,
     pg_set_decimal_point__doc__},
    {"get_decimal", (PyCFunction)pg_get_decimal, METH_NOARGS,
     pg_get_decimal__doc__},
    {"set_decimal", (PyCFunction)pg_set_decimal, METH_O,
     pg_set_decimal__doc__},
    {"get_bool", (PyCFunction)pg_get_bool, METH_NOARGS, pg_get_bool__doc__},
    {"set_bool", (PyCFunction)pg_set_bool, METH_VARARGS, pg_set_bool__doc__},
    {"get_array", (PyCFunction)pg_get_array, METH_NOARGS, pg_get_array__doc__},
    {"set_array", (PyCFunction)pg_set_array, METH_VARARGS,
     pg_set_array__doc__},
    {"set_query_helpers", (PyCFunction)pg_set_query_helpers, METH_VARARGS,
     pg_set_query_helpers__doc__},
    {"get_bytea_escaped", (PyCFunction)pg_get_bytea_escaped, METH_NOARGS,
     pg_get_bytea_escaped__doc__},
    {"set_bytea_escaped", (PyCFunction)pg_set_bytea_escaped, METH_VARARGS,
     pg_set_bytea_escaped__doc__},
    {"get_jsondecode", (PyCFunction)pg_get_jsondecode, METH_NOARGS,
     pg_get_jsondecode__doc__},
    {"set_jsondecode", (PyCFunction)pg_set_jsondecode, METH_O,
     pg_set_jsondecode__doc__},
    {"cast_array", (PyCFunction)pg_cast_array, METH_VARARGS | METH_KEYWORDS,
     pg_cast_array__doc__},
    {"cast_record", (PyCFunction)pg_cast_record, METH_VARARGS | METH_KEYWORDS,
     pg_cast_record__doc__},
    {"cast_hstore", (PyCFunction)pg_cast_hstore, METH_O,
     pg_cast_hstore__doc__},
    ...
    {"get_pqlib_version", (PyCFunction)pg_get_pqlib_version, METH_NOARGS,
     pg_get_pqlib_version__doc__},
    {NULL, NULL} /* sentinel */
};
...
static struct PyModuleDef moduleDef = {
    PyModuleDef_HEAD_INIT, "_pg", /* m_name */
    pg__doc__,                    /* m_doc */
    -1,                           /* m_size */
...
PyInit__pg(void)
{
    ...
    connType.tp_base = noticeType.tp_base = queryType.tp_base =
        sourceType.tp_base = &PyBaseObject_Type;
    largeType.tp_base = &PyBaseObject_Type;

    if (PyType_Ready(&connType) || PyType_Ready(&noticeType) ||
        PyType_Ready(&queryType) || PyType_Ready(&sourceType) ||
    ...
    InvalidResultError =
        PyErr_NewException("pg.InvalidResultError", DataError, NULL);
    PyDict_SetItemString(dict, "InvalidResultError", InvalidResultError);

    NoResultError =
        PyErr_NewException("pg.NoResultError", InvalidResultError, NULL);
    ...
    LargeObject = (PyObject *)&largeType;
    PyDict_SetItemString(dict, "LargeObject", LargeObject);
    ...
    PyDict_SetItemString(dict, "TRANS_ACTIVE",
                         PyLong_FromLong(PQTRANS_ACTIVE));
    PyDict_SetItemString(dict, "TRANS_INTRANS",
                         PyLong_FromLong(PQTRANS_INTRANS));
    PyDict_SetItemString(dict, "TRANS_INERROR",
                         PyLong_FromLong(PQTRANS_INERROR));
    PyDict_SetItemString(dict, "TRANS_UNKNOWN",
                         PyLong_FromLong(PQTRANS_UNKNOWN));

    /* Polling results */
    PyDict_SetItemString(dict, "POLLING_OK",
                         PyLong_FromLong(PGRES_POLLING_OK));
    PyDict_SetItemString(dict, "POLLING_FAILED",
                         PyLong_FromLong(PGRES_POLLING_FAILED));
    PyDict_SetItemString(dict, "POLLING_READING",
                         PyLong_FromLong(PGRES_POLLING_READING));
    PyDict_SetItemString(dict, "POLLING_WRITING",
                         PyLong_FromLong(PGRES_POLLING_WRITING));
